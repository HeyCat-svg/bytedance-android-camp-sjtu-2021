...
public class Clock extends View {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (!threadFlag) {
            threadFlag = true;
            new ClockHandler("clock_flush_handler").start();
        }
    }
    ...
    private class ClockHandler extends HandlerThread implements Handler.Callback {
        public static final int MSG_FLUSH_CLOCK = 100;

        private Handler mHandler;

        public ClockHandler(String name) {
            super(name);
        }
        ...
        protected void onLooperPrepared() {
            ...
            mHandler.sendEmptyMessage(MSG_FLUSH_CLOCK);
        }

        @Override
        public boolean handleMessage(@NonNull Message msg) {
            if (msg.what == MSG_FLUSH_CLOCK) {
                Log.e("Draw Action", "draw action");
                invalidate();
                mHandler.sendEmptyMessageDelayed(MSG_FLUSH_CLOCK, 1000);
            }
